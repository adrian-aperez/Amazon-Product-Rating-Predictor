...
with tab5:
    ...
    # Guardar caso en el historial
    try:
        if 'predicted_rating' in locals():
            if st.button("💾 Guardar este caso"):
                try:
                    # Crear un registro con los datos actuales
                    reviews_text = "Más de 60" if st.session_state.reviews >= 60 else "Menos de 60"
                    registro = {
                        "Rating": round(st.session_state.predicted_rating, 2),
                        "Descripción": st.session_state.product_description,
                        "Precio": st.session_state.price,
                        "Reseñas": reviews_text,
                    ...
                    st.error(f"⚠️ Error al guardar el caso: {e}")
        else:
            st.info("Realiza una predicción primero para generar el informe del producto.")
    except Exception as e:
        st.error(f"⚠️ No se pudo generar el informe del producto: {e}")
                    ...
                    if st.button("🖨 PDF"):
                        try:
                            def generate_pdf(df):
                                ...
                                x_start = 30
                                y_start = 700
                                column_widths = [50, 180, 50, 60, 100]  # Anchos personalizados
                                ...
                                for _, row in df.iterrows():
                                    ...
                                    for i, value in enumerate(row):
                                        ...
                                        if i == 1 and len(text) > 30:
                                            parts = [text[j:j+30] for j in range(0, len(text), 30)]
